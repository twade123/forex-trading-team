...
def _check_decision_quality(cr: Dict, instrument: str) -> List[Dict]:
    ...
    # Normalize: validator prompts spec 0-10 integer scale. Downstream expects 0.0-1.0.
    # Integer scale: conf=7 = 70% tradeable conviction. Prior /5 was a bug.
    _conf_normalized = val_conf / 10.0 if isinstance(val_conf, (int, float)) and val_conf > 1 else val_conf
    if isinstance(_conf_normalized, (int, float)) and _conf_normalized > 0.7 and action == "hold":
        _conf_display = f"{int(val_conf)}/10" if val_conf > 1 else f"{val_conf:.0%}"
...
def _check_consecutive_holds(instrument: str) -> List[Dict]:
    ...
    try:
        ...
        conn = sqlite3.connect(fr_path, isolation_level=None)
