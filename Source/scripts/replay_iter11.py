"""replay_iter11.py — Production-faithful replay with iter 10 prompt.

Uses ghost_replay's exact production-faithful pipeline:
- vision_training_data.input_prompt (production JSON: pair, alert_type, indicators,
  fan_state, bb_expanding, narrative)
- vision_training_data.chart_path (production-saved chart PNG)
- VALIDATOR_TOOLS.md + pattern_library.md (production skill files)
- iter 10 prompt = ghost_validator_v1.md + EARLY-ENTRY rule + EXHAUSTION READ
  + RETRACE READ (knowledge added to prompt; no answers pre-computed in input)
...
Run:
    ...
    python3 scripts/replay_iter11.py
"""
...
ITER10_PROMPT = "/tmp/prompt_variants/iter11_anchor_rebuilt.md"
SKILLS_DIR = "<repo_root>/Skills"
SKILL_FILES = []  # iter 6 proved skills hurt; iter 9 won without them
LOCAL_ENDPOINT = "http://127.0.0.1:11502/v1/chat/completions"
LOCAL_MODEL_NAME = "mlx-community/Qwen3.5-35B-A3B-4bit"

OUT_RESULTS = "/tmp/iter11_results.json"
OUT_LOG = "/tmp/iter11_replay.log"
...
def load_system_prompt() -> str:
    parts = [Path(ITER10_PROMPT).read_text().strip()]
...
def main():
    ...
    log("=" * 70)
    log("ITER 10 — production-faithful replay, iter11 prompt (+ early-entry + exhaustion + retrace)")
    log("=" * 70)
    ...
    for trade_id, vtd_id, pair, direction, actual_pips, category in COHORT:
        ...
        log(f"  Chart loaded={'YES' if chart_b64 else 'NO'} ({entry['chart_path']})")
    ...
    log(f"ITER 10 SUMMARY: {correct_count}/{len(COHORT)} correct "
