"""replay_iter12.py — iter 9 exact formula against the 14-trade extended cohort.

iter 9 won 9/10 on the 10-trade cohort with this exact setup:
- System prompt: iter2_no_narrative_tax.md (full ghost_validator_v1.md minus
...
Run:
    ...
    python3 scripts/replay_iter12.py
"""
...
ITER12_PROMPT = "/tmp/prompt_variants/iter2_no_narrative_tax.md"
...
OUT_RESULTS = "/tmp/iter12_results.json"
OUT_LOG = "/tmp/iter12_replay.log"
...
def load_system_prompt() -> str:
    parts = [Path(ITER12_PROMPT).read_text().strip()]
...
def main():
    ...
    log("=" * 70)
    log("ITER 12 — iter 9 exact formula on 14-trade extended cohort")
    log("=" * 70)
    log(f"Prompt: {ITER12_PROMPT}")
    ...
    log(f"ITER 12 SUMMARY: {correct_count}/{len(COHORT)} correct "
