"""replay_iter14.py — iter 9 exact formula against the 14-trade extended cohort.

iter 9 won 9/10 on the 10-trade cohort with this exact setup:
- System prompt: iter14_strict_rsi_exhaustion.md (full ghost_validator_v1.md minus
...
Run:
    ...
    python3 scripts/replay_iter14.py
"""
...
ITER14_PROMPT = "/tmp/prompt_variants/iter14_strict_rsi_exhaustion.md"
...
OUT_RESULTS = "/tmp/iter14_results.json"
OUT_LOG = "/tmp/iter14_replay.log"
...
def load_system_prompt() -> str:
    parts = [Path(ITER14_PROMPT).read_text().strip()]
...
def main():
    ...
    log("=" * 70)
    log("ITER 14 — iter 9 exact formula on 14-trade extended cohort")
    log("=" * 70)
    log(f"Prompt: {ITER14_PROMPT}")
    ...
    log(f"ITER 14 SUMMARY: {correct_count}/{len(COHORT)} correct "
