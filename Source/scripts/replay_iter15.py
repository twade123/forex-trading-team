"""replay_iter15.py — iter 9 exact formula against the 14-trade extended cohort.

iter 9 won 9/10 on the 10-trade cohort with this exact setup:
- System prompt: iter15_multisignal_exhaustion.md (full ghost_validator_v1.md minus
...
Run:
    ...
    python3 scripts/replay_iter15.py
"""
...
ITER15_PROMPT = "/tmp/prompt_variants/iter15_multisignal_exhaustion.md"
...
OUT_RESULTS = "/tmp/iter15_results.json"
OUT_LOG = "/tmp/iter15_replay.log"
...
def load_system_prompt() -> str:
    parts = [Path(ITER15_PROMPT).read_text().strip()]
...
def main():
    ...
    log("=" * 70)
    log("ITER 15 — iter 9 exact formula on 14-trade extended cohort")
    log("=" * 70)
    log(f"Prompt: {ITER15_PROMPT}")
    ...
    log(f"ITER 15 SUMMARY: {correct_count}/{len(COHORT)} correct "
