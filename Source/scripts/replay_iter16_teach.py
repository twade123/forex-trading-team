"""replay_iter16.py — iter 9 exact formula against the 14-trade extended cohort.

iter 9 won 9/10 on the 10-trade cohort with this exact setup:
- System prompt: iter16_structural_first.md (full ghost_validator_v1.md minus
...
Run:
    ...
    python3 scripts/replay_iter16.py
"""
...
ITER16_PROMPT = "/tmp/prompt_variants/iter16_structural_first.md"
...
OUT_RESULTS = "/tmp/iter16_teach_results.json"
OUT_LOG = "/tmp/iter16_teach_replay.log"

# Production teaching image for local 35B (matches agents/trading_cycle.py:_load_local_validator_images)
TEACHING_IMAGE_PATH = "<repo_root>/Data/charts/teaching/tim_teach_eurchf_annotated_short_snipe.png"
TEACHING_DESCRIPTION = (
    ...
    "zone marked — the mirror pattern applies to bullish setups."
)


def load_teaching_b64():
    if not os.path.exists(TEACHING_IMAGE_PATH):
        return None, "image/png"
    raw = open(TEACHING_IMAGE_PATH, "rb").read()
...
COHORT = [
    ...
    # NEW Friday 5/8 cohort (3 winners + 2 losers)
    ("13765", 3639, "GBP_JPY", "BUY",   29.3, "winner"),
    ("13809", 3666, "GBP_USD", "BUY",   -5.1, "loser"),
    ("13817", 3671, "EUR_JPY", "BUY",    5.1, "winner"),
    ("13827", 3673, "EUR_USD", "BUY",    4.7, "winner"),
    ("13843", 3682, "AUD_JPY", "BUY",   -7.7, "loser"),
]


def load_system_prompt() -> str:
    parts = [Path(ITER16_PROMPT).read_text().strip()]
...
def call_35b(system_prompt: str, task_text: str, chart_b64, chart_media: str = "image/png",
             teach_b64=None, teach_media: str = "image/png") -> str:
    content = []
    # Production order: teaching image FIRST, then live chart, then task text
    if teach_b64:
        content.append({"type": "text", "text": f"TEACHING REFERENCE: {TEACHING_DESCRIPTION}"})
        content.append({"type": "image_url",
                        "image_url": {"url": f"data:{teach_media};base64,{teach_b64}"}})
    if chart_b64:
        content.append({"type": "text", "text": "LIVE CHART (this is the chart to evaluate):"})
        content.append({"type": "image_url",
...
def main():
    ...
    log("=" * 70)
    log("ITER 16 — iter 9 exact formula on 14-trade extended cohort")
    log("=" * 70)
    log(f"Prompt: {ITER16_PROMPT}")
    ...
    teach_b64, teach_media = load_teaching_b64()
    log(f"Teaching image loaded={'YES' if teach_b64 else 'NO'} ({TEACHING_IMAGE_PATH})")
    ...
    for trade_id, vtd_id, pair, direction, actual_pips, category in COHORT:
        ...
        try:
            t_call = time.time()
            raw = call_35b(system_prompt, task, chart_b64, chart_media, teach_b64, teach_media)
            dt = time.time() - t_call
    ...
    log(f"ITER 16 TEACH SUMMARY (19-trade, with teaching): {correct_count}/{len(COHORT)} correct "
