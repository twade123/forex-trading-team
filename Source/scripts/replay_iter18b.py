"""replay_iter18b.py — iter 18 re-test with two stacked changes:

1. Anchor-aware pivot prompt: explicitly invokes pattern_17 from the model's
   training (fade R1/S1 with reversal, ride R2/S2 break with momentum) rather
   than generic 'reaction zones' framing.
2. Session-gate awareness: indicator block reports BLOCKED/OPEN; prompt rule
   downgrades phase 3 TRADE_NOW to WATCH when BLOCKED.

Same 19 pivot charts as iter 18 (regenerated by oanda_chart_pivot_regen).
Same cohort, same 35B endpoint. Apples-to-apples comparison vs iter 18
(12/19 strict, 4 BAD) and iter 16 v2 baseline (13/19 strict, 4 BAD).

Run:
    cd "<repo_root>/Source"
    source ~/myenv/bin/activate
    python3 scripts/replay_iter18b.py
"""
...
PROMPT_PATH = "/tmp/prompt_variants/iter18b_anchored.md"
...
OUT_RESULTS = "/tmp/iter18b_results.json"
OUT_LOG = "/tmp/iter18b_replay.log"
...
def build_task_text(pair, direction, indicator_block):
    pd = pair.replace("_", "/")
    base = (
...
def score(category, verdict, direction, trade_dir):
...
def main():
    ...
    log("=" * 70)
    log("ITER 18B — anchor-aware pivot prompt + session gate")
    log("=" * 70)
    log(f"Prompt: {PROMPT_PATH}")
    log(f"Indicator blocks: {INDICATOR_BLOCKS_JSON} (now includes Session gate)")
    log(f"Chart source: /tmp/replay_charts_pivot (regenerated by iter 18)")
    ...
    for trade_id, pair, direction, entry_iso, actual_pips, category in COHORT:
        ...
        if not os.path.exists(chart_out):
            chart_out = regenerate_chart_with_pivots(pair, entry_iso, chart_out)
            if not chart_out:
                ...
                results.append({
                    ...
                    "reasoning_snippet": "regen failed", "correct": False, "bucket": "BAD",
                })
                continue
        chart_b64, chart_media = load_chart_b64(chart_out)
        sess = "BLOCKED" if ind.get("session_blocked") else "OPEN"
        log(f"  Chart: {chart_out} ({os.path.getsize(chart_out)//1024}KB) | session={sess}")
    ...
    log(f"ITER 18B SUMMARY: IDEAL={ideal}  OK={ok}  BAD={bad}  Acceptable={ideal+ok}/19")
    log(f"  Strict score (old): {correct_count}/19")
    log(f"  Baseline iter 16 v2: 13/19 strict | 10 IDEAL + 5 OK + 4 BAD = 15/19 acceptable")
    log(f"  Iter 18 pivot only:  12/19 strict |  9 IDEAL + 6 OK + 4 BAD = 15/19 acceptable")
