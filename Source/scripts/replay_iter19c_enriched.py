"""replay_iter19_patterns.py — iter 19: pattern detection + chart labels +
dynamic prompt section with verbatim pattern_library.md quotes.

Stack of changes vs iter 16 v2 baseline:
1. Swing-trace overlay (red/green dots + connecting line)
2. Pattern detectors fire for each chart (engulfing, hammer, doji, BB squeeze,
   triangles, channel, star — all tunable via pattern_detectors.DETECTOR_ENABLED)
3. Detected patterns are LABELED on the chart at their bar index using the
   exact pattern name from pattern_library.md
4. Prompt dynamically includes a "DETECTED PATTERNS" section with VERBATIM
   pattern_library.md entries for each fired pattern
5. Session-gate awareness (same as iter 18b)

All detectors can be toggled individually via DETECTOR_ENABLED in
scripts/pattern_detectors.py — kill them one at a time to isolate impact.

Run:
    cd "<repo_root>/Source"
    source ~/myenv/bin/activate
    python3 scripts/replay_iter19_patterns.py
"""
...
PROMPT_PATH = "/tmp/prompt_variants/iter19_swings.md"
...
OUT_RESULTS = "/tmp/iter19c_results.json"
OUT_LOG = "/tmp/iter19c_replay.log"
PATTERN_CHART_DIR = "/tmp/replay_charts_pattern_enriched"
...
def main():
    ...
    log("=" * 70)
    log("ITER 19 — pattern detectors + chart labels + verbatim quotes")
    log("=" * 70)
    log(f"Prompt: {PROMPT_PATH}")
    log(f"Detectors enabled: {DETECTOR_ENABLED}")
    log(f"Chart source: {PATTERN_CHART_DIR} (regenerated with pattern overlay)")
    ...
    for trade_id, pair, direction, entry_iso, actual_pips, category in COHORT:
        ...
        try:
            ...
            results.append({"trade_id": trade_id, "pair": pair, "direction": direction,
                            ...
                            "reasoning_snippet": str(e), "correct": False, "bucket": "BAD",
    ...
    log(f"ITER 19 SUMMARY: IDEAL={ideal}  OK={ok}  BAD={bad}  Acceptable={ideal+ok}/19")
    log(f"  Baseline iter 16 v2: 10 IDEAL + 5 OK + 4 BAD = 15/19 acceptable")
    log(f"  Iter 18 pivot only:   9 IDEAL + 6 OK + 4 BAD = 15/19 acceptable")
