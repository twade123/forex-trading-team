"""replay_iter20c.py — iter 20c: continuation-vs-exhaustion structural rule.
...
9. **NEW iter 20c**: HARD RULE inserted before RETRACE section —
   SELL with price still below E21 + EMA fan parallel or lightly contracting +
   BBs parallel or lightly contracting = CONTINUATION setup. Deep RSI alone
   (even <20 or >80) is INSUFFICIENT to call SKIP. Distance from E100 is also
   INSUFFICIENT. Only candle crossing back to E21 / fan converging heavily /
   BB collapsing back to squeeze gates a downgrade. Targets 13362 BAD
   (RSI<20 exhaustion misread on phase=2 with intact below-E21 structure).

Run:
    cd "<repo_root>/Source"
    source ~/myenv/bin/activate
    python3 scripts/replay_iter20c.py
"""
...
PROMPT_PATH = "/tmp/prompt_variants/iter20c.md"
...
OUT_RESULTS = "/tmp/iter20c_results.json"
OUT_LOG = "/tmp/iter20c_replay.log"
PATTERN_CHART_DIR = "/tmp/replay_charts_pattern_20a"
